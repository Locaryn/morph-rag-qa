//! Stdio MCP server shipped by plugin-rag-qa.
use locaryn_plugin_rag_qa::{answer_question, index_document, IndexDocumentRequest, RagQueryRequest};
...
async fn handle_request(request: Value) -> Value {
    ...
    match method {
        "initialize" => success(id, json!({
            ...
            "capabilities": { "tools": {} },
            "serverInfo": { "name": "plugin-rag-qa", "version": VERSION }
        })),
...
async fn call_tool(name: &str, args: Value) -> Result<Value, String> {
    match name {
        "index_document" => {
            let req: IndexDocumentRequest = serde_json::from_value(args)
                .map_err(|e| format!("Paramètres invalides: {e}"))?;
            let res = index_document(req).await?;
            Ok(json!(res))
        }
        "answer_question" => {
            let req: RagQueryRequest = serde_json::from_value(args)
                .map_err(|e| format!("Paramètres invalides: {e}"))?;
